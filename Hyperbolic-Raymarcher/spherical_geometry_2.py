...
def geodesic(p, v, t):
    if not np.isclose(lorentzian_inner_product(p,p), -1):
        raise ValueError("Position must lie on manifold")
    r = np.linalg.norm(v)
    if np.isclose(r, 0):
        raise ValueError("Direction must be non-zero")
    if not np.isclose(lorentzian_inner_product(p,v), 0):
        raise ValueError("Direction must be tangent to sphere at position")
    return np.cosh(t)*p + np.sinh(t)*v/r

def distance(p, q):
   return np.arccosh(-np.lorentzian_inner_product(p, q),)
